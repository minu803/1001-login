...
class AuthManager {
    ...
    async signInWithCredentials(email, password, rememberMe) {
        // Use your custom login endpoint
    ...
    async signInWithEmail(email, options = {}) {
        // For now, redirect to registration if it's a signup, otherwise show error
        if (options.isSignup) {
            // Create account with your register endpoint
            const result = await fetch('/api/auth/register', {
                ...
                body: JSON.stringify({
                    email,
                    name: options.name,
                    password: 'temp-password-' + Math.random().toString(36) // Generate temp password
                })
            });
            ...
            this.showSuccess('Account created successfully! Please use the demo login for now.');
            setTimeout(() => this.closeModal(), 3000);
        } else {
            // For login without password, show message to use demo or enter password
            this.showError('Please enter your password or use one of the demo accounts.');
        }
    }
    ...
    async handleDemoLogin(email) {
        ...
        try {
            // Map demo emails to actual test users
            const emailMap = {
                'admin@demo.1001stories.com': 'admin@test.com',
                'volunteer@demo.1001stories.com': 'volunteer@test.com',
                'teacher@demo.1001stories.com': 'volunteer@test.com', // Use volunteer for teacher demo
                'student@demo.1001stories.com': 'learner@test.com'
            };

            const actualEmail = emailMap[email] || email;
            
            // Use your login endpoint with password '123'
            const result = await fetch('/api/auth/login', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
