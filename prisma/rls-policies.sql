...
CREATE POLICY "volunteer_points_admin_only" ON volunteer_points
...
-- Allow all authenticated users to view available rewards
CREATE POLICY "volunteer_rewards_select_all" ON volunteer_rewards
  FOR SELECT TO authenticated
  USING (true);

-- Only admins can manage rewards
CREATE POLICY "volunteer_rewards_admin_only" ON volunteer_rewards
  FOR ALL TO authenticated
  USING (is_admin())
  WITH CHECK (is_admin());
...
-- Allow all authenticated users to view active quests
...
-- Allow all authenticated users to view active projects
...
CREATE POLICY "volunteer_certificates_admin_only" ON volunteer_certificates
  FOR ALL TO authenticated
  USING (is_admin())
  WITH CHECK (is_admin());
...
GRANT SELECT, INSERT, UPDATE ON volunteer_applications TO authenticated;
GRANT SELECT, UPDATE ON quest_assignments TO authenticated;
GRANT SELECT, INSERT, UPDATE ON volunteer_evidence TO authenticated;
GRANT SELECT ON volunteer_points TO authenticated;
GRANT SELECT ON volunteer_rewards TO authenticated;
GRANT SELECT, INSERT, UPDATE ON volunteer_redemptions TO authenticated;
GRANT SELECT, UPDATE ON mentor_relations TO authenticated;
GRANT SELECT ON volunteer_matches TO authenticated;
GRANT SELECT, INSERT, UPDATE ON quest_reviews TO authenticated;
GRANT SELECT ON quests TO authenticated;
GRANT SELECT ON volunteer_projects TO authenticated;
GRANT SELECT, INSERT, UPDATE ON volunteer_hours TO authenticated;
GRANT SELECT ON volunteer_certificates TO authenticated;

-- Grant full access to all tables for admin operations
...
COMMENT ON TABLE volunteer_rewards IS 'Volunteer rewards with RLS: all authenticated users can view, only admins can modify';
